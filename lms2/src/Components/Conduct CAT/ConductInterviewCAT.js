...
function ConductInterviewCAT() {
    ...
    const [usedSubSkills, setUsedSubSkills] = useState(new Set()); // Add this new state
...
useEffect(() => {
  ...
  const handleAddQuestion = (question) => {
    ...
    // If 5 or more questions are selected for this sub-skill, add it to usedSubSkills
    const questionsForCurrentSubSkill = selectedQuestions.filter(q => 
      q.subSkillId === selectedSubSkill._id
    ).length;
    
    if (questionsForCurrentSubSkill + 1 >= 5) {
      setUsedSubSkills(prev => new Set([...prev, selectedSubSkill._id]));
    }
  };
  ...
  const handleRemoveQuestion = (questionId) => {
    ...
    // Clean up ratings state
    const updatedRatings = { ...questionRatings };
    delete updatedRatings[questionId];
    setQuestionRatings(updatedRatings);
    
    // Check if we should remove the sub-skill from usedSubSkills
    const remainingQuestionsForSubSkill = selectedQuestions.filter(q => 
      q.subSkillId === removedQuestion.subSkillId && q._id !== questionId
    ).length;
    
    if (remainingQuestionsForSubSkill < 5) {
